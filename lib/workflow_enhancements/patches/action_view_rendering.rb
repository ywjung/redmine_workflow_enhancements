...
module RenderWorkflowEnhancements
  def render(*args, &block)
    if args.first == :super
      ...
      # previously render_without_workflow_enhancements options
      view_stack.pop
      result
    else
      options = args.first
      ...
      # previously render_without_workflow_enhancements (*args, &block)
...
module ActionView
  class Base
    ...
    Base.send(:prepend, RenderWorkflowEnhancements) #alias_method_chain :render, :workflow_enhancements
    
    #def render_with_workflow_enhancements(*args, &block)... moved to RenderWorkflowEnhancements now called render()
